# -*- coding: utf-8 -*-
"""MINMAX- Human Vs AI.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1HiIGTII2i3CQcEgy2cIUT1rug1pZBEZE
...
##Alpha-Beta Pruning (Min-Max) on Connect-Four - Human VS AI
...
"""Some other variables are initialized, human's turn is given the value 0 and AI's turn is given the value 1. Human's game piece is given the value 1 and AI's game piece is given the value 2."""
...
*   piece: represents the game piece (e.g., player's piece or AI's piece) that will be placed on the board
"""
...
def evaluate_window(window, piece):
    opponent_piece = PLAYER_PIECE if piece == AI_PIECE else AI_PIECE #determines the opponent's game piece based on the provided piece
...
"""This function is used to evaluate the overall score of a game board position for a specific game piece. It starts by examining the central column to determine the presence of the specified piece and rewards the AI for having more of its pieces in the center. Next, it evaluates the board in all directions, including rows, columns, and diagonals, by creating 4-cell "windows" and passing them to the evaluate_window function. The evaluate_window function assesses each window, considering the number of consecutive pieces of the specified type and empty spaces, and adjusts the score accordingly."""
...
def minimax(board, depth, alpha, beta, maximizing_player):
    ...
    if depth == 0 or is_terminal: #checks if depth is zero or is terminal state reached
        if is_terminal:
            if winning_move(board, AI_PIECE): #if the game is won by AI, it returns a high positive score
                return (None, 10000000)
            elif winning_move(board, PLAYER_PIECE): #f the game is won by the opponent, it returns a high negative score
                return (None, -10000000)
...
turn = random.randint(PLAYER_TURN, AI_TURN) #randomly selects who will do the first move
...
while not game_over: #loop that continues as long as the game_over variable is False
    if turn == PLAYER_TURN and not_over: #Human's turn
        print("Player's turn[X]")
        col = int(input("Select a column (0-6): ")) #allows user to input his/her move
        if is_valid_location(board, col): #if it is a valid move
            row = get_next_open_row(board, col)
            drop_piece(board, row, col, PLAYER_PIECE) #it places the move on the game board
            if winning_move(board, PLAYER_PIECE): #checks if winning move is reached
                print("HUMAN WINS!")
                print_board(board)
                not_over = False
                break
            print_board(board)
            turn = AI_TURN #switches turn to AI
    elif turn == AI_TURN and not game_over and not_over: #AI's turn
        print("AI's turn[O]")
        col, minimax_score = minimax(board, 5, -math.inf, math.inf, True) #gets the AI's move using the alpa-beta pruning function
        if is_valid_location(board, col): #if it is a valid move
            row = get_next_open_row(board, col)
            drop_piece(board, row, col, AI_PIECE) #it places the move on the game board
            if winning_move(board, AI_PIECE): #checks if winning move is reached
                print("AI WINS!")
                print_board(board)
                not_over = False
                break
            print_board(board)
            turn = PLAYER_TURN #switches turn to Human